...
1. **假设** 用户在显示 "BTCUSDT" 品种的 TradingView 图表页面，**当** 用户点击插件弹窗中的 "4h" 按钮，**则** 图表切换到 4 小时周期，图表完全加载后，自动下载名为 "BTCUSDT_4h_20251217_143052.png" 的截图。
...
### 用户故事 2 - 快速多周期分析 (优先级: P2)

作为进行多时间周期分析的交易者，我希望能够快速截取同一品种在多个时间周期的截图，以便比较不同时间框架的图表形态。

**优先级原因**: 这扩展了核心功能，满足需要跨时间周期进行全面分析的用户需求，这是常见的交易工作流程。

**独立测试**: 可通过依次点击多个周期按钮并验证每个截图以正确的周期信息下载来完整测试。

**验收场景**:

1. **假设** 用户想要截取 15m、1h 和 4h 周期的图表，**当** 用户依次点击每个按钮，**则** 下载三个独立的截图，文件名中包含各自的周期标识。

2. **假设** 用户在前一个截图仍在处理时点击新的周期按钮，**当** 系统检测到正在进行的操作，**则** 插件显示忙碌状态的视觉反馈，并将下一个请求加入队列。
...
- **FR-001**: 插件必须提供带有预设时间周期按钮（15m、1h、4h、1D、1W）的弹窗界面。
